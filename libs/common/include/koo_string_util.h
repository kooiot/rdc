...
inline char k_str_get_alpha(wchar_t n)
{
...
inline std::string k_str_trim(const std::string& str)
{
...
inline std::string k_str_trim_end(const std::string& str, char trimchar)
{
...
inline std::string k_str_trim_ex(const std::string& str, char trimchar)
{
...
inline std::vector<std::string> k_str_split_ex(const std::string& s, const std::string& delim)
{
...
inline int k_str_split(const std::string& str, std::vector<std::string>& ret_, std::string sep = ",")
{
	if (str.empty())
	{
		return 0;
...
inline std::string k_str_replace(const std::string& src, const std::string& replacement, const std::string& dest)
{
	std::string ret;
...
#define USE_BOOST_LOCALE
#endif
inline std::string k_str_to_lower(const std::string &str)
{
...
inline bool k_str_compare_no_case(const std::string &strA, const std::string &strB)
{
	std::string str1 = k_str_to_lower(strA);
	std::string str2 = k_str_to_lower(strB);
	return (str1 == str2);
}
#ifdef USE_BOOST_LOCALE
#include "boost/locale.hpp"

inline std::wstring k_str_to_wstr(const std::string& s, const char* loc = "GBK")
{
	std::wstring result = boost::locale::conv::to_utf<wchar_t>(s, loc);
	return result;
}

inline std::string k_wstr_to_str(const std::wstring& s, const char* loc = "GBK")
{
	std::string result = boost::locale::conv::from_utf(s, loc);
	return result;
}
inline std::string k_str_to_utf8(const std::string& str, const char* loc = "GBK")
{
...
inline std::string k_utf8_to_string(const std::string & str, const char* loc = "GBK")
{
...
inline std::string k_wstr_to_utf8(const std::wstring& wstr, const char* loc = "GBK")
{
	std::string str = k_wstr_to_str(wstr, loc);
	std::string result = k_str_to_utf8(str.c_str(), loc);
	return result;
}
#else
inline std::string k_str_to_utf8_(const char* str)
{
	std::wstring wstr = k_str_to_wstr(str);
...
inline std::string k_utf8_to_string_(const std::string & s)
{
	std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
	std::wstring wstr = conv.from_bytes(s);
	std::string result = k_wstr_to_str(wstr);
	return result;
}
inline std::wstring k_str_to_wstr(const std::string& s)
{
...
inline std::string k_wstr_to_str(const std::wstring& s)
{
	setlocale(LC_ALL, "zh-CN");
	int ln = s.length();
...
inline std::string k_str_to_utf8(const char* str)
{
	std::wstring wstr = k_str_to_wstr(str);
	//std::locale old_locale;
	//std::locale utf8_locale(old_locale, new codecvt_utf8<wchar_t>);
...
inline std::string k_utf8_to_string(const std::string & s)
{
	std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
	std::wstring wstr = conv.from_bytes(s);
	std::string result = k_wstr_to_str(wstr);
...
inline std::string k_wstr_to_utf8(const wchar_t* wideStr)
{
	//std::locale old_locale;
	//std::locale utf8_locale(old_locale, new codecvt_utf8<wchar_t>);
	//std::locale::global(utf8_locale);
...
inline std::string k_str_gb_to_alpha(const std::string& str)
{
	//std::wstring ws = k_str_to_wstr(str, "");
	std::string result;
	for (int i = 0; i < (int)str.size();)
	{
		...
		else
		{
			uint16_t w = (*ptr) << 8;
			w |= *(ptr + 1);
			char x = k_str_get_alpha((wchar_t)w);
			result.push_back(x);
			i += 2;
		}
	}
