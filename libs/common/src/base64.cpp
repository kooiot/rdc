...
{
	ZBase64 zb64;
	std::string ss = zb64.Encode((uint8_t*)text.c_str(), text.size());
	for (auto& c : ss)
	{
		c ^= x;
	}
	std::string enc = zb64.Encode((uint8_t*)ss.c_str(), ss.size());
	return enc;
}
...
std::string base64_dec_ex(const std::string& enc, uint8_t x)
{
	ZBase64 zb64;
	int outbytes = 0;
	std::string ss = zb64.Decode(enc.c_str(), enc.size(), outbytes);
	for (auto& c : ss)
	{
		c ^= x;
	}
	std::string dec = zb64.Decode(ss.c_str(), ss.size(), outbytes);
	return dec;
}
...
std::vector<uint8_t> base64_dec_bin(const std::string& str)
{
	...
	std::string ret = zb64.Decode(str.c_str(), str.size(), outbytes);
